...
def analyze_tweet_sentiment(tweet):
    ...
    for word in tweet_words(tweet):
        ...
        if has_sentiment(word_sentiment):
            value += sentiment_value(word_sentiment) 
            count += 1 
    if count > 0:
...
def find_centroid(polygon):
    ...
    (1.0, 2.0, 0.0)
    """
    centroidX, centroidY, sum_area = 0, 0, 0
    tempX = latitude(polygon[0])
    tempY = longitude(polygon[0])
    for point in polygon[1:]:
        tempX_next, tempY_next = latitude(point), longitude(point)
        sum_area += (tempX * tempY_next - tempX_next * tempY)
        centroidX += (tempX + tempX_next) * (tempX * tempY_next - tempX_next * tempY)
        centroidY += (tempY + tempY_next) * (tempX * tempY_next - tempX_next * tempY) 
        tempX, tempY = tempX_next, tempY_next
    sum_area = sum_area / 2

    # Speical Case - Area is 0
    if sum_area == 0:
        return (latitude(polygon[0]), longitude(polygon[0]), sum_area)

    return (centroidX / (6 * sum_area), centroidY / (6 * sum_area), abs(sum_area)) # Area cannot be negative
...
def find_state_center(polygons):
    ...
    return make_position(sumX / sumA, sumY / sumA)
...
def group_tweets_by_state(tweets):
    ...
    for tweet in tweets:
        ...
        # By us_states.items(), all values in dictionary will be tuple(key, [vlaue]). min function will compare first elements of turple which will be the smallest distance.
        mystate = min((geo_distance(location, find_state_center(state_geo)), state_name) for state_name, state_geo in us_states.items())[1] # [1] is for state_name
        
        #Style 2
        """
        Alternative code for Style 1. This is much longer but probably more readable.

        mystate = None # default
        distance = None # default
        location = tweet_location(tweet)

        for state_name in us_states: #us_states are dictionary
            new_distance = geo_distance(location, find_state_center(us_states[state_name])) # Compares the current location to the center of the state
            if distance is None or distance > new_distance:
                distance = new_distance
                mystate = state_name
        """
        
        if mystate not in tweets_by_state:
            tweets_by_state[mystate] = []
        tweets_by_state[mystate].append(tweet)
...
def average_sentiments(tweets_by_state):
    ...
    for state in tweets_by_state:
        ...
        for tweet in tweets_by_state[state]:
            each_sentiment = analyze_tweet_sentiment(tweet)
            if has_sentiment(each_sentiment):
                count += 1
                sum_sentiments += sentiment_value(each_sentiment)
        if count != 0:
            averaged_state_sentiments[state] = sum_sentiments / count
